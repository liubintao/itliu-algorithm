...
/**
 * @desc 给定一个字符串str，给定一个字符串类型的数组arr。
...
public class Code02_StickersToSpellWord {
    ...
    private static int process1(Map<String, Integer> dp, int[][] map, String rest) {
        ...
        int n = map.length; //一共有多少种贴纸
        int[] tmap = new int[26]; // tmap替代rest
        ...
        for (int i = 0; i < n; i++) {
            ...
            // i 贴纸， j 枚举a~z字符
            for (int j = 0; j < 26; j++) {
                if (tmap[j] > 0) { //target里包含这个字符
                    //在target中消掉当前贴纸中所有的tmap[j]字符还剩几个当前字符就遍历几次，都添加到sb
